...
class Drive(Dataset):
    ...
    def __init__(self, base_dir=None, split='train', num=None, transform=None):
        ...
        train_path = self._base_dir+'training/'
        test_path = self._base_dir+'test/'
        
        self._data_dir = self._base_dir + 'converted_data/'
        
        if split=='train':
            self.path = train_path
            self.label_list = os.listdir(self.path + '1st_manual/')
            self.label_list = [os.path.splitext(label_name)[0] for label_name in self.label_list]
    ...
    def __getitem__(self, idx):
        ...
        h5f = h5py.File(self._data_dir + image_name + ".h5", 'r')
        image = h5f['image'][:]
        label = h5f['label'][:]
